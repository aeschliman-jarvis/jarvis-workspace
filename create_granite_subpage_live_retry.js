...
(async() => {
  ...
  await page.keyboard.type('Transformation Research Snapshot', { delay: 15 }).catch(()=>{});
  await page.waitForTimeout(1200);
  await page.keyboard.press('Enter').catch(()=>{});
  await page.waitForTimeout(2500);

  // If a new untitled or target page is open, populate it.
  const body = await page.locator('body').innerText().catch(()=> '');
  if (!/Transformation Research Snapshot/i.test(body)) {
  ...
  for (const line of lines) {
    ...
    if (isHeading) {
      await page.keyboard.type('/heading 2', { delay: 4 }).catch(()=>{});
