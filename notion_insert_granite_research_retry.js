...
(async() => {
  ...
  const page = context.pages().find(p => p.url().includes('Jarvis-Hub-3362225c1eac80cfa382dda618acb112')) || context.pages().find(p => p.url().includes('notion.so/Jarvis-Hub')) || context.pages().find(p => p.url().includes('notion.so'));
  if (!page) throw new Error('No Notion page found');
  ...
  if (!/Granite Transformations/i.test(body)) throw new Error('Granite Transformations link not visible on current page');

  await page.locator('text=Granite Transformations').first().click({ force: true, timeout: 10000 });
  ...
  for (const p of context.pages()) {
    try {
      ...
      if (/Granite Transformations/i.test(title) || /^Granite Transformations/m.test(txt)) {
        target = p;
      }
    } catch {}
  }
  ...
  await target.keyboard.type(content, { delay: 4 });
